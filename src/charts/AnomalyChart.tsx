...
export function AnomalyChart({ ds, metric, yearMin, yearMax, method }: Props) {
  ...
  const { anom, trend, annual } = useMemo(() => {
    if (pts.length < 20) return { anom: [], trend: null, annual: [] };
    const clim = harmonicFitByDoy(pts.map((p) => p.doy), pts.map((p) => p.v), 4);
    const anom = pts.map((p) => ({ t: p.decyear, year: p.year, a: p.v - clim.predict(p.doy) }));
  ...
  return (
    <div ref={ref} className="w-full">
      <svg width={width} height={height} role="img">
        ...
        {anom.map((d, i) => (
          <circle key={i} cx={x(d.t)} cy={y(d.a)} r={1.1} fill="#999" fillOpacity={0.35} />
        ))}
        <polyline points={annual.map((d) => `${x(d.year + 0.5)},${y(d.a)}`).join(" ")}
          fill="none" stroke="#2e7d32" strokeWidth={1.6} />
        {annual.map((d) => (
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-1 px-1 text-xs text-ink/70">
        <span className="inline-flex items-center gap-1.5">
          <span style={{ background: "#2e7d32", height: 2, width: 16 }} /> annual mean anomaly
        </span>
        <span className="inline-flex items-center gap-1.5">
